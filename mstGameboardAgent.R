...
flog.appender(appender.file(paste(mripPath,"/MINTlogs/corMstAgent.log",sep="")), name=logNS)
...
MainComputeMst <- function(inputDfJson){
  ...
  save("mstLength",file=paste(mripPath,"/wd/latestMstLength.rdata",sep=""))
  ...
  if(marketIndicator == "open"){
    ...
    write.csv.ffdf(as.ffdf(as.ff(t(writeCorMat))),file=paste(mripPath,"/wd/correlationsTable.csv",sep=""),append=TRUE)
    
    write.csv.ffdf(as.ffdf(as.ff(t(writeMstMat))),file=paste(mripPath,"/wd/mstTable.csv",sep=""),append=TRUE)
