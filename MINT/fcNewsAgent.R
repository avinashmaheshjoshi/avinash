...
flog.appender(appender.file(paste(mripPath,"/MINTlogs/fcNews_info.log",sep="")), name="info_news")
flog.threshold(ERROR, name="error_news")
flog.appender(appender.file(paste(mripPath,"/MINTlogs/fcNews_error.log",sep="")), name="error_news")
...
makeNewsJson <- function(newsObj){
  ...
  json <- paste("{\"fcNews\":[{\"title\":", "\"", dfr[1,1], "\"",",\"link\":", "\"",dfr[1,2], "\"",",\"date\":", "\"",dfr[1,3],"\"", ",\"desc\":", "\"",dfr[1,4],"\"", "}", sep="")
  for(i in 2:nrow(dfr)){   
  ...
  if(errorCode != 600){
	...
	if(length(financialcontentUrls!=0)){
      ...
      return(json)
	    ...
	    if(newsJson == ""){
	      flog.error("The fc Urls's not found", name= "error_news")
		  errorCode <<- 0
