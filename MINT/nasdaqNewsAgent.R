...
flog.appender(appender.file(paste(mripPath,"/MINTlogs/nasdaqNews_info.log",sep="")), name="info_news")
flog.threshold(ERROR, name="error_news")
flog.appender(appender.file(paste(mripPath,"/MINTlogs/nasdaqNews_error.log",sep="")), name="error_news")
...
tryCatch({load("/home/musigma/MRIP/wd/urlList.RData")}, error=function(err){errorCode<<-600;flog.error("RData file not found", name= "error_news")}, finally={})
#________________________________________________________________________________________
...
makeNewsJson <- function(newsObj){
  ...
  json <- paste("{\"nasdaqNews\":[{\"title\":", "\"", dfr[1,1], "\"",",\"link\":", "\"",dfr[1,2], "\"",",\"date\":", "\"",dfr[1,3],"\"", ",\"desc\":", "\"",dfr[1,4],"\"", "}", sep="")
  for(i in 2:nrow(dfr)){   
...
getNasdaqNews <- function(){ 
  ...
  if(length(nasdaqUrls!=0)){
    ...
    return(json)
	    ...
	    if(newsJson == ""){
	      flog.error("The nasdaq Urls's not found", name= "error_news")
		  errorCode <<- 0
