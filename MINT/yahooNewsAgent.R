...
flog.appender(appender.file(paste(mripPath,"/MINTlogs/yahooNews_info.log",sep="")), name="info_news")
flog.threshold(ERROR, name="error_news")
flog.appender(appender.file(paste(mripPath,"/MINTlogs/yahooNews_error.log",sep="")), name="error_news")
...
makeNewsJson <- function(newsObj){
  ...
  json <- paste("{\"yahooNews\":[{\"title\":", "\"", dfr[1,1], "\"",",\"link\":", "\"",dfr[1,2], "\"",",\"date\":", "\"",dfr[1,3],"\"", ",\"desc\":", "\"",dfr[1,4],"\"", "}", sep="")
  for(i in 2:nrow(dfr)){   
...
getYahooNews <- function(){ 
  ...
  if(length(yahooUrls!=0)){
    ...
    return(json)
	    ...
	    if(newsJson == ""){
	      flog.error("The Yahoo Urls's not found", name= "error_news")
		  errorCode <<- 0
