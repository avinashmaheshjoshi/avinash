...
dbProperties2 <- suppressWarnings(as.vector(as.matrix(read.table("db.properties", header = F, sep = ",", skip = 1))))
...
if(errorCode==201){
  symbolVec <- suppressWarnings(as.vector(as.matrix(read.table("prediction.properties", header = F, sep = ",", skip = 3))))
}
...
prevJson<-""
activeMQIp <- 'tcp://162.192.100.46:61616'
frameSize <- 2
...
dbProperties <- suppressWarnings(as.vector(as.matrix(read.table("db.properties", header = F, sep = ","))))
...
mint_main_fn = function(){
  ...
  if(retFrameCount  >= returnsDfFrameSize){
    ...
    if(length(correlationOnes) != 0){
      pearson_mat[correlationOnes] = abs(pearson_mat[correlationOnes])-0.01
    }
